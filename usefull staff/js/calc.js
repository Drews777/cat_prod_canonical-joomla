...
var centerizeVert = function(b, c, d, f) {
    function e(a) { var b = { top: ~~((a.parent().height() - a.outerHeight()) / 2 + g) };
        0 > $.inArray(a.css("position"), ["absolute", "relative"]) && (b.position = "relative");
        a.css(b) }
    b = $(b); if (b.length) { var g = c ? ~~c : 0;
        b.each(function() { var a = $(this); if (a.height()) { if (e(a), f) { var b = a.css("top"),
                        c = setInterval(function() { e(a);
                            a.css("top") !== b && clearInterval(c) }, 20);
                    $(window).load(function() { clearInterval(c) }) } } else if (a.is("img")) { a.hide(); var d = setInterval(function() { a.height() && (clearInterval(d), a.show(), e(a)) }, 20) } });
        d && $(window).load(function() { centerizeVert(b, c) }) } };
...
function domReady() {
    ...
    (function() {
        ...
        function calc() {
            ...
            if (dataLightFront) total += 2500; // цена тонировки передних фар;
            if (dataLightBack) total += 2500; // цена тонировки задних фар;
            if (dataBody == 'uni') total = Math.round(total * 1.1); // 1.1 - 10% наценка за универсал;
